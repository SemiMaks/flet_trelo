...
class TrelloApp(UserControl):
    def __init__(self, page: Page, store: DataStore):
        ...
        self.login_profile_button = PopupMenuItem(text="Вход", on_click=self.login)
        self.appbar_items = [
        ...
        self.appbar = AppBar(
            ...
            leading_width=100,
            title=Text(f"Трело", font_family="Pacifico", size=32, text_align="start"),
            center_title=False,
            toolbar_height=75,
    ...
    def initialize(self):
        ...
        if len(self.boards) == 0:
            self.create_new_board("Моя первая доска")
        self.page.go("/")

    def login(self, e):
        def close_dlg(e):
            if user_name.value == "" or password.value == "":
                user_name.error_text = "Ваше имя"
                password.error_text = "Ваш пароль"
                self.page.update()
                return
            else:
        ...
        dialog = AlertDialog(
            title=Text("Пожалуйста введите ваше имя"),
            content=Column(
                [
                    user_name,
                    password,
                    ElevatedButton(text="Имя", on_click=close_dlg),
                ],
                tight=True,
            ),
